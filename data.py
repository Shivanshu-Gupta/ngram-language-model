...
def read_texts(data_dir, dname):
    ...
    with open(f'{data_dir}/{dname}.train.txt', 'r', errors='replace') as inf:
        train_txt = inf.readlines()
    with open(f'{data_dir}/{dname}.test.txt', 'r', errors='replace') as inf:
        test_txt = inf.readlines()
    with open(f'{data_dir}/{dname}.dev.txt', 'r', errors='replace') as inf:
        dev_txt = inf.readlines()

    from sklearn.feature_extraction.text import CountVectorizer
    count_vect = CountVectorizer()
    count_vect.fit(train_txt)
    tokenizer = count_vect.build_tokenizer()
    class Data: pass
    data = Data()
    data.train = []
